    ...
    <div className="p-5 ">
      ...
      {teamsAttendance.length === 0 ? (
        ...
        <div className="overflow-auto">
          {teamsAttendance.map((team,index) => (
            <div key={team._id} className="bg-white p-5 rounded-md shadow-md mb-4">
              <h2 className="text-xl font-semibold mb-2 ">{index+1}. {team.teamName}</h2>
              <table className="table-auto w-full text-left border-collapse border border-gray-300">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="border border-gray-300 px-4 py-2">Name</th>
                    <th className="border border-gray-300 px-4 py-2">regNumber</th>
                    ...
                    <th className="border border-gray-300 px-4 py-2">Fourth Attendance</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-gray-300 px-4 py-2">{team.lead.name}</td>
                    <td className="border border-gray-300 px-4 py-2">{team.lead.regNumber}</td>
                    ...
                    <td className="border border-gray-300 px-4 py-2">
                        {team.lead.FourthAttd || "Not Marked"}
                      </td>
                  </tr>
                  {team.members.map((member) => (
                    <tr key={member.name}>
                      <td className="border border-gray-300 px-4 py-2">{member.name}</td>
                      <td className="border border-gray-300 px-4 py-2">{member.regNumber}</td>
                      <td className="border border-gray-300 px-4 py-2">
